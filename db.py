...
def listar_portais_projeto() -> list:
    ...
    validade, fases) diretamente em SQL, para nunca ter de ler as imagens/
    PDFs em base64 embutidos em `conteudo_markdown` só para montar uma
    lista."""
    with get_conn() as conn:
        with conn.cursor() as cur:
            cur.execute(
            ...
            return [{"id": l["id"], "card_id": l["card_id"], "titulo": l["titulo"],
                    ...
                    "validade": l["validade"], "fases": l["fases"] or []} for l in linhas]
