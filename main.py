...
def portais_projeto_regenerar_todos():
    """TEMPORÁRIO (remover depois de correr uma vez): volta a renderizar
    todos os portais já gerados com o template atual (sem tocar em nenhum
    dado do projeto) — ver tools/portal_projeto.regenerar_todos_portais_projeto.
    Usado depois de mudar a cor dos blocos de resumo, para essa alteração
    aparecer também nos portais já enviados às clientes."""
...
def documento_gerado_eliminar(id: int):
    ...
    "Eliminar" da página de listagem de portais — ver
    tools/portal_projeto.pagina_lista). Ação irreversível: o link que a
    cliente tinha deixa de funcionar."""
    if not eliminar_documento_gerado(id):
        raise HTTPException(status_code=404, detail="documento não encontrado")
...
def planeamento_ecos_largos_logistica_quem_carrega(corpo: dict = Body(...)):
    """Define/limpa quem carrega uma OF — ver
    tools/planeamento_serracao.definir_quem_carrega."""
    basecamp_card_id = corpo.get("basecamp_card_id")
    if not basecamp_card_id:
        return JSONResponse({"erro": "falta indicar basecamp_card_id"}, status_code=400)
